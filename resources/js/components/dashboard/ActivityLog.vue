<template>
  <div class="bg-white br2 pa3 z-0" style="box-shadow: 0 1px 1px #ccc;">
    ...
    <div class="entry-milestone relative mb3" style="padding-left: 3rem;">
      <div v-for="logData in activityLogsData.data">
        <div class="milestone mt3 mb2 black-60">
          <svg class="icon mr3" viewBox="0 0 14 16" version="1.1" width="14" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M10.86 7c-.45-1.72-2-3-3.86-3-1.86 0-3.41 1.28-3.86 3H0v2h3.14c.45 1.72 2 3 3.86 3 1.86 0 3.41-1.28 3.86-3H14V7h-3.14zM7 10.2c-1.22 0-2.2-.98-2.2-2.2 0-1.22.98-2.2 2.2-2.2 1.22 0 2.2.98 2.2 2.2 0 1.22-.98 2.2-2.2 2.2z"></path></svg>{{ logData.logDate }}
        </div>

        <ol v-for="(log, index) in logData.logs" class="entry-list list ma0 pa0 relative">
          <li class="ba b--black-10" v-if="index === Object.keys(logData.logs).length - 1">
            <event-activity :log="log" :index="index" v-if="log.feedable_type === 'App\\Event'"></event-activity>
            <default-event-activity :log="log" :index="index" v-if="log.feedable_type === 'App\\DefaultEvent'"></default-event-activity>
          </li>

          <li class="bt bl br b--black-10" v-if="index !== Object.keys(logData.logs).length - 1">
            <event-activity :log="log" :index="index" v-if="log.feedable_type === 'App\\Event'"></event-activity>
            <default-event-activity :log="log" :index="index" v-if="log.feedable_type === 'App\\DefaultEvent'"></default-event-activity>
          </li>
        </ol>
    ...
    <div class="load-more tc" v-if="shouldShowLoadMore">
      <button v-if="!loadingMore" @click="loadMore()" class="btn default-btn fw6 pv1 ph3 mv2 br4">
        {{ __('dashboard.load_more') }}
      </button>
      <button v-else class="btn default-btn fw6 pv1 ph3 mv2 br4 disabled">
      ...
      loadMore() {
        ...
        axios.get('/activity-log?page=' + (this.activityLogsData.current_page + 1))
          .then(response => {
            ...
            // Push more rows(data of new page) to collection.
