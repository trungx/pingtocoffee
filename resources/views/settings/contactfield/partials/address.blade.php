...
<div class="pa3 mb3 br2 bg-white" style="box-shadow: 0 1px 1px #ccc;">
  <div class="header relative mb-2">
    <h6>{{ __('settings.address_type') }}</h6>
    <a href="/settings/contactfield/create?type=address" class="default-btn pv1 ph3 b absolute top-0 right-0">
      {{ __('settings.add_new_address_cta') }}
    </a>
  </div>
  @if (isset($contactFieldValuesGrouped['address']) && $contactFieldValuesGrouped['address']->count() > 0)
    <ul class="list pa0 ma0">
      @foreach($contactFieldValuesGrouped['address'] as $contactFieldValue)
  ...
  @else
    <div class="mv2">
      <div class="tc pa2 light-gray-text">
        ...
        {{ __('settings.address_empty_desc') }}
      </div>
    </div>
  @endif
</div>
